...
public class ShadowBorder_MY implements Border {
	...
	public static ShadowBorder_MY newInstance() {
		return newInstance(DEFAULT_SIZE, DEFAULT_COLOR, DEFAULT_ALPHA);
	}

	public static ShadowBorder_MY newInstance(int size) {
		return newInstance(size, DEFAULT_COLOR, DEFAULT_ALPHA);
	}

	public static ShadowBorder_MY newInstance(int size, Color c) {
		return newInstance(size, c, DEFAULT_ALPHA);
	}

	public static ShadowBorder_MY newInstance(int size, Color c, float alpha) {
	...
	public static ShadowBorder_MY newInstance(int size, Color c, float alpha, float paddingLeft, float paddingTop) {
		size = Math.max(size, 0);
		alpha = Math.max(alpha, 0f);
		alpha = Math.min(alpha, 1f);
		...
		return new ShadowBorder_MY(size, c, alpha, paddingLeft, paddingTop);
	}
	...
	public static class Builder {
		...
		public ShadowBorder_MY build() {
			return new ShadowBorder_MY(size, color, alpha, paddingLeft, paddingTop);
		}
	}
	...
	private ShadowBorder_MY(int size, Color color, float alpha, float paddingLeft, float paddingTop) {
		assert size > 0;
		assert color != null;
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
		...
		shadowArea.subtract(new Area(new Rectangle(
				bounds.x + insets.left, bounds.y + insets.top, bounds.width - insets.left - insets.right,
...
//	}
	...
	private static enum Edge {
		...
		Left {
			...
			@Override
			void fill(ShadowBorder_MY b, Graphics2D g, Rectangle r) {
